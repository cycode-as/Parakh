 ...
 * normalize.js
 *
 * Converts a raw Gemini response object into the canonical AnalysisResult shape.
 *
 * Handles three classes of Gemini inconsistencies:
...
function normalizeSuccessProbability(v, trust, fit) {
  ...
  // Handle numeric form (Gemini sometimes returns 0-100)
  const n = Number(v);
  if (!Number.isNaN(n)) {
    if (n >= 70) return 'high';
...
/**
 * Normalise a raw parsed Gemini JSON object into a valid AnalysisResult.
 *
 ...
 */
export function normalizeGeminiResponse(raw) {
